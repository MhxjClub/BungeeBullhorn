...
public class bbh extends Command {
    ...
    public void execute(CommandSender sender, String[] arg) {
        if(arg.length==0){
            sender.sendMessage("§3Bungee§bBullhorn §fv"+ lkq2.instance.getDescription().getVersion() + "By §3Kazuha§bAyato");
            sender.sendMessage("§7§o//君问归期未有期,巴山夜雨涨秋池");
            sender.sendMessage("§3/bhorn help §f帮助页面");
            if(sender.hasPermission("bhorn.use")){
                sender.sendMessage("§3/"+ lkq2.config.getString("command") + " <内容> §f喊话");
            ...
            if(sender.hasPermission("bhorn.pm")){
                sender.sendMessage("§3/pm <目标> <消息> §f发送私聊");
            }
             if(sender.hasPermission("bhorn.op")){
                sender.sendMessage("§3/bhorn reload §f重载配置");
                sender.sendMessage("§f插件版本:§3" + lkq2.instance.getDescription().getVersion());
            }
        }
        if(arg.length < 1) {
            return;
